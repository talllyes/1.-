...
app.controller('MapController', function ($scope, NgMap, $http) {
    ...
    mapBase.url = {                                 //樣版網址
        ...
        mapControl: "template/mapControl.html",
    ...
    //取得選取點位全部版本ID
    mapBase.getMarkerVer = function (event, id, my) {
    ...
    //新增路口點位
    mapBase.insertMaker = function (type) {
        if (type == "edit") {
            ...
            var newPosition = mapBase.map.markers['marker'].getPosition() + "";
            newPosition = newPosition.replace("(", "").replace(")", "");
            $http({
                method: 'post',
                url: "json/PostUpdateBase.ashx?type=insertIntersectionBase",
                data: newPosition
            }).success(function (data, status, headers, config) {
                mapBase.mapBaseMakers.push({ Position: newPosition, IntersectionID: data, Img: "img/map.png" });
                postToastr("新增路口點位成功");
                mapBase.centerChanged();
            }).error(function (data, status, headers, config) {
            ...
            $("#insertMarkerCheck").css("top", "150px");
            mapBase.newMakerShow = false;
            mapBase.centerChanged();
        }
    }
    ...
    mapBase.openPage = function (type) {
        if (type == "roadBaseData") {
            $("#roadBaseDatali").addClass("kai-active");
            $("#noticeli").removeClass("kai-active");
            $("#jobli").removeClass("kai-active");
            $('#roadBaseDataPage').collapse('show');
            $('#noticePage').collapse('hide');
            $('#jobPage').collapse('hide');
        } else if (type == "notice") {
            mapBase.getMakerNotice(mapBase.selectMarker.IntersectionID);
            $("#noticeli").addClass("kai-active");
            $("#roadBaseDatali").removeClass("kai-active");
            $("#jobli").removeClass("kai-active");
            $('#noticePage').collapse('show');
            $('#roadBaseDataPage').collapse('hide');
            $('#jobPage').collapse('hide');
        } else if (type == "job") {
            $("#jobli").addClass("kai-active");
            $("#roadBaseDatali").removeClass("kai-active");
            $("#noticeli").removeClass("kai-active");
    ...
    mapBase.getPermission = function () {
        $http({
            method: 'post',
            url: "json/GetPostUser.ashx?type=getMapPermission"
        }).success(function (data, status, headers, config) {
            mapBase.Premission = data;
            mapBase.draggable = data.ItemName[1].Permission;
        }).error(function (data, status, headers, config) {
            postToastr("取得使用者資料失敗，請聯絡系統管理者！", "error");
        });
    }

    mapBase.getPermission();
    ...
    mapBase.editMy = function (type) {
        if (type == "edit") {
            mapBase.edit.UserName = mapBase.Premission.UserName;
            mapBase.edit.Password = "";
            $('#editMyAD').modal('show');
        } else if (type == "ok") {
            $http({
                method: 'POST',
                url: "json/GetPostUser.ashx?type=updateMyUser",
                data: mapBase.edit
            }).success(function (data, status, headers, config) {
                if (data == "ok") {
                    $('#editMyAD').modal('hide');
                    mapBase.Premission.UserName = mapBase.edit.UserName;
                } else {
                    alert(data);
                }
            }).error(function (data, status, headers, config) {
